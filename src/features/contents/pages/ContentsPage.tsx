...
import { 
  ...
  ChevronRight, Send, Clock, Archive, Lock, Unlock,
  Loader2, Calendar, User, X,
  CalendarCheck, FileBarChart, ListTodo, AlertCircle
} from 'lucide-react';
...
export default function ContentsPage() {
  ...
  const [capsuleStep, setCapsuleStep] = useState<'info' | 'select_friend' | 'write'>('info');
  ...
  // 3. 친구 목록 로드
  useEffect(() => {
    if (!user || !isCapsuleModalOpen) return;
    const fetchFriends = async () => {
      const { data } = await supabase
        .from('friendships')
        .select(`friend:users!friendships_friend_id_fkey(id, nickname, profile_image)`)
        .eq('user_id', user.id)
        .eq('status', 'accepted');
      ...
      if (data) {
        setFriends(data.map((f: any) => ({
          id: f.friend.id,
          nickname: f.friend.nickname || '알 수 없음',
          profile_image: f.friend.profile_image
        })));
      }
    };
    fetchFriends();
  }, [user, isCapsuleModalOpen]);
  ...
  const sendTimeCapsule = async () => {
    if (!user || !selectedFriend || !capsuleContent || !unlockDate) return;
    const toastId = toast.loading('캡슐 전송 중...');
    ...
    try {
      ...
      let roomId = [user.id, selectedFriend.id].sort().join("_");
      const { data: roomCheck } = await supabase.from('chat_rooms').select('id').eq('id', roomId).maybeSingle();
      if (!roomCheck) {
         await supabase.from('chat_rooms').insert([{ id: roomId, type: 'individual', members_count: 2, title: selectedFriend.nickname, created_by: user.id }]);
         await supabase.from('room_members').insert([{ room_id: roomId, user_id: user.id }, { room_id: roomId, user_id: selectedFriend.id }]);
      }
      
      await supabase.from('messages').insert({
        room_id: roomId,
        sender_id: user.id,
        content: `⏳ 타임캡슐 도착! 개봉일: ${new Date(unlockDate).toLocaleString()}`,
        is_read: false
      });
  ...
  return (
    <div className="h-full w-full flex flex-col bg-[#000000] text-white">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-10 pb-24">
          ...
          {!hasTimeCapsuleAccess ? (
            ...
            <div className="space-y-4">
              <button
                ...
                disabled={isPaymentLoading}
                className="w-full bg-[#1C1C1E] border border-white/10 rounded-2xl p-5 flex items-center justify-between active:bg-[#252525] transition-colors"
              >
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-[#2C2C2E] rounded-xl flex items-center justify-center text-orange-500">
                    <Send className="w-6 h-6" />
                  </div>
                  <div className="text-left">
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className="font-bold text-white text-lg">새 캡슐 보내기</p>
                      <span className="text-[11px] font-bold text-orange-500 bg-orange-500/10 px-2 py-0.5 rounded border border-orange-500/20">₩4,900</span>
                    </div>
                    <p className="text-sm text-gray-400">친구에게 감동을 전하세요</p>
                  </div>
                </div>
              ...
              <div className="bg-[#1C1C1E] rounded-2xl border border-[#2C2C2E] overflow-hidden">
                <div className="flex border-b border-[#2C2C2E]">
                  <button onClick={() => setActiveTab('sent')} className={`flex-1 py-4 text-sm font-medium ${activeTab === 'sent' ? 'text-white bg-[#252525]' : 'text-gray-500'}`}>보낸 캡슐</button>
                  <div className="w-[1px] bg-[#2C2C2E]" />
                  <button onClick={() => setActiveTab('received')} className={`flex-1 py-4 text-sm font-medium ${activeTab === 'received' ? 'text-white bg-[#252525]' : 'text-gray-500'}`}>받은 캡슐</button>
                </div>
                <div className="p-3 min-h-[150px]">
                  {isDataLoading ? (
                    <div className="flex flex-col items-center justify-center h-40 text-gray-500 gap-2"><Loader2 className="w-6 h-6 animate-spin text-orange-500" /></div>
                  ) : (
                    <div className="space-y-3">
                      {(activeTab === 'sent' ? sentCapsules : receivedCapsules).length === 0 ? (
                        <div className="text-center py-12 text-gray-500"><Archive className="w-10 h-10 mx-auto mb-3 opacity-20" /><p className="text-sm">캡슐이 없습니다.</p></div>
                      ) : (
                        (activeTab === 'sent' ? sentCapsules : receivedCapsules).map(c => {
                          ...
                          return (
                            <div key={c.id} onClick={() => !locked && activeTab === 'received' && navigate(`/time-capsule/view/${c.id}`)} className={`p-4 rounded-xl border flex items-center justify-between ${locked ? 'bg-[#151515] border-[#252525] text-gray-500' : 'bg-[#252525] border-orange-500/20 text-white cursor-pointer'}`}>
                              <div className="flex items-center gap-4">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${locked ? 'bg-[#2C2C2E]' : 'bg-orange-500/10'}`}>{locked ? <Lock className="w-5 h-5" /> : <Unlock className="w-5 h-5 text-orange-500" />}</div>
                                <div><p className="text-base font-bold mb-0.5">{name}</p><p className="text-xs opacity-70 flex items-center gap-1.5"><Clock className="w-3 h-3" /> {getTimeRemaining(c.unlock_at)}</p></div>
                              </div>
                              {activeTab === 'sent' && canEdit(c) && <button onClick={(e) => { e.stopPropagation(); navigate(`/time-capsule/edit/${c.id}`); }} className="text-xs font-bold bg-[#3A3A3C] px-3 py-1.5 rounded-lg">수정</button>}
                            </div>
                          );
                        })
                      )}
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-left">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  {!hasReportAccess && <span className="text-[11px] font-bold text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20">₩2,900</span>}
                </div>
                <p className="text-sm text-gray-400">관계 정밀 분석 및 맞춤형 솔루션</p>
              </div>
            </div>
            {isPaymentLoading && paymentTarget === 'report' ? <Loader2 className="w-6 h-6 text-purple-500 animate-spin" /> : <ChevronRight className="w-6 h-6 text-gray-600 group-hover:text-white" />}
          </button>
        ...
        {/* 모달은 1. 타임캡슐 최초 해제 시에만 사용되거나 제거됨. 현재 로직상 '새 캡슐 보내기'는 바로 페이지 이동이므로 모달 코드 제거하여 깔끔하게 정리 */}
      </div>
    </div>
  );
}
...
function FeatureCard({ icon, title, desc }: { icon: React.ReactNode, title: string, desc: string }) {
  return (
    <div className="bg-[#1C1C1E] border border-[#2C2C2E] rounded-2xl p-5 relative opacity-50 pointer-events-none">
      <div className="w-10 h-10 bg-[#252525] rounded-xl flex items-center justify-center text-gray-400 mb-3">{icon}</div>
      <h3 className="text-white font-bold text-base mb-1">{title}</h3>
      <p className="text-xs text-gray-500">{desc}</p>
      <div className="absolute top-4 right-4 px-2 py-1 bg-[#252525] rounded text-[10px] text-gray-500 border border-[#333] font-medium">준비중</div>
    </div>
  );
}