import { useState, useRef } from 'react'; // ✨ useCallback 제거
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { Camera, Check, Image as ImageIcon, X, Eye, User as UserIcon } from 'lucide-react'; // ✨ Trash2 제거
...
export default function ProfileSetupPage({ onComplete }: ProfileSetupPageProps) {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      // ✨ 부모로부터 받은 onComplete가 있으면 실행, 없으면 직접 이동
      if (onComplete) {
        onComplete();
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-dark-bg text-white relative overflow-hidden">
      ...
      <div className="flex-1 px-6 pt-20 pb-40 overflow-y-auto custom-scrollbar">
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            <input type="text" value={nickname} onChange={e => setNickname(e.target.value)} placeholder="친구들이 부를 이름" className="w-full bg-[#1C1C1E] rounded-2xl py-4 px-5 text-white focus:ring-2 focus:ring-brand-DEFAULT border border-[#2C2C2E] outline-none transition-all placeholder-[#48484A]" />
          </div>
          <div className="space-y-2">
            <label className="text-[13px] font-bold text-[#636366] ml-1 uppercase tracking-wider">Status Message</label>
            <input type="text" value={statusMessage} onChange={e => setStatusMessage(e.target.value)} placeholder="오늘의 기분이나 인사말" className="w-full bg-[#1C1C1E] rounded-2xl py-4 px-5 text-white focus:ring-2 focus:ring-brand-DEFAULT border border-[#2C2C2E] outline-none transition-all placeholder-[#48484A]" />
          </div>
      ...
      <div className="fixed bottom-0 left-0 w-full p-6 pb-safe bg-gradient-to-t from-dark-bg via-dark-bg to-transparent z-10">
        <button onClick={handleComplete} disabled={!nickname} className={`w-full h-15 rounded-2xl font-black text-lg flex items-center justify-center gap-2 shadow-2xl transition-all active:scale-[0.98] ${nickname ? 'bg-brand-DEFAULT text-white shadow-brand-DEFAULT/20' : 'bg-[#2C2C2E] text-[#48484A] cursor-not-allowed'}`}>그레인 시작하기 {nickname && <Check className="w-6 h-6 ml-1" />}</button>
      </div>
        ...
        {isPreviewOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-[#1C1C1E] w-full max-w-[340px] rounded-[40px] overflow-hidden shadow-2xl border border-[#2C2C2E]">
              ...
              <div className="h-64 bg-[#2C2C2E] relative overflow-hidden">{backgroundUrl && <img src={backgroundUrl} className="w-full h-full object-cover" alt="Pre" />}<div className="absolute inset-0 bg-gradient-to-b from-transparent to-black/60" /></div>
              <div className="px-6 pb-10 -mt-16 relative z-10 flex flex-col items-center">
                <div className="w-28 h-28 rounded-[38px] border-4 border-[#1C1C1E] bg-[#2C2C2E] overflow-hidden mb-5 shadow-2xl">{avatarUrl ? <img src={avatarUrl} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-[#3A3A3C] flex items-center justify-center"><UserIcon className="w-10 h-10 opacity-20" /></div>}</div>
                <h3 className="text-2xl font-black text-white mb-2">{nickname || '그레인 친구'}</h3>
                <p className="text-[#8E8E93] text-sm text-center leading-relaxed max-w-[80%] break-keep">{statusMessage || '설정된 상태 메시지가 없습니다.'}</p>
              </div>
            </motion.div>
          </div>
        )}
      ...
      <AnimatePresence>
        {isCropOpen && tempImageSrc && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black flex flex-col">
            <div className="h-16 flex items-center justify-between px-5 bg-black/80 backdrop-blur-md z-10 sticky top-0"><button onClick={() => setIsCropOpen(false)} className="p-2 -ml-2"><X className="w-7 h-7 text-white" /></button><span className="font-bold text-lg text-white">이미지 편집</span><button onClick={handleCropSave} className="px-5 py-2 bg-brand-DEFAULT rounded-full font-black text-sm text-white shadow-lg active:scale-95 transition-all">완료</button></div>
            <div className="relative flex-1 bg-black"><Cropper image={tempImageSrc} crop={crop} zoom={zoom} aspect={currentImageType === 'avatar' ? 1 : 16/9} onCropChange={setCrop} onCropComplete={(_, p) => setCroppedAreaPixels(p)} onZoomChange={setZoom} cropShape={currentImageType === 'avatar' ? 'round' : 'rect'} showGrid={false} /></div>
            <div className="h-24 bg-black/80 backdrop-blur-md flex items-center justify-center px-10 gap-4"><span className="text-xs text-[#8E8E93]">ZOOM</span><input type="range" min={1} max={3} step={0.1} value={zoom} onChange={(e) => setZoom(Number(e.target.value))} className="flex-1 accent-brand-DEFAULT" /></div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
