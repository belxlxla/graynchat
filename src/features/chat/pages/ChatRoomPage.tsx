...
export default function ChatRoomPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`room_messages_${chatId}`, { config: { broadcast: { self: false } } });
    ...
    channel.on('postgres_changes',
      ...
      (payload) => {
        ...
        if (updated.user_id === user.id && updated.wallpaper !== undefined) setBackground(updated.wallpaper || '');
      }
    );
    ...
    channel.subscribe((status) => {
      if (status === 'SUBSCRIBED') console.log('[Realtime] ✅ 연결 성공:', chatId);
      else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
        console.error('[Realtime] 연결 실패:', status);
        setTimeout(() => { if (channelRef.current) { supabase.removeChannel(channelRef.current); channelRef.current = null; } }, 3000);
      }
    });
  ...
  const handleSendMessage = async () => {
    ...
    const tempMessage: Message = {
      id: Date.now(), room_id: chatId, sender_id: user.id,
      content: textToSend, created_at: new Date().toISOString(),
      is_read: false, tempId, isRetrying: false, isFailed: false,
    };
    ...
    try {
      const { data: inserted, error: sendError } = await supabase
        .from('messages').insert({ room_id: chatId, sender_id: user.id, content: textToSend, is_read: false })
        .select().single();
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const tempMessage: Message = {
      id: Date.now(), room_id: chatId, sender_id: user.id,
      content: localUrl, created_at: new Date().toISOString(),
      is_read: false, tempId, isRetrying: true, isFailed: false,
    };
    setMessages(prev => [...prev, tempMessage]);
    scrollToBottom('smooth');
    ...
    try {
      ...
      const { data: newMsg, error: insertError } = await supabase.from('messages')
        .insert({ room_id: chatId, sender_id: user.id, content: publicUrl, is_read: false }).select().single();
      ...
      supabase.from('chat_rooms').update({ last_message: '파일을 보냈습니다.', last_message_at: new Date().toISOString() }).eq('id', chatId).then();
  ...
  const handleRetryMessage = async (msg: Message) => {
    ...
    try {
      const { data: inserted, error } = await supabase.from('messages')
        .insert({ room_id: chatId!, sender_id: user!.id, content: msg.content, is_read: false }).select().single();
    ...
    setShowRetryModal(false); setFailedMessage(null);
  };
  ...
  const handleAddFriend = async () => {
    ...
    try {
      ...
      await supabase.from('friends').upsert({
        ...
        name: friendUser?.name || roomTitle, avatar: friendUser?.avatar || roomAvatar,
        status: friendUser?.status_message || null, friendly_score: 50, is_blocked: false,
      });
      setIsFriend(true); setIsBlocked(false);
      toast.success('친구로 추가되었습니다.');
      fetchInitialData();
    } catch { toast.error('친구 추가에 실패했습니다.'); }
  };
  ...
  const handleBlockUser = async () => {
    ...
    try {
      ...
      await supabase.from('friends').upsert({ user_id: user.id, friend_user_id: friendId, name: friendUser?.name || roomTitle, is_blocked: true });
      setIsBlocked(true); toast.success('사용자가 차단되었습니다.');
    } catch { toast.error('차단에 실패했습니다.'); }
  };
  ...
  const searchResults = useMemo(() => {
    ...
    return messages.filter(m => m.content && getFileType(m.content) === 'text' && m.content.toLowerCase().includes(q)).map(m => m.id);
  }, [searchQuery, messages]);

  useEffect(() => {
    if (searchResults.length > 0 && currentSearchIndex === -1) {
      setCurrentSearchIndex(0);
      setTimeout(() => { messageRefs.current[searchResults[0]]?.scrollIntoView({ behavior: 'smooth', block: 'center' }); }, 100);
    }
  ...
  const renderMessageContent = (msg: Message, isMe: boolean) => {
    ...
    if (type === 'image') {
      return (
        <div className={`rounded-[18px] overflow-hidden max-w-[220px] cursor-pointer relative border ${
    ...
    if (type === 'video') {
      return (
        <div className={`rounded-[18px] overflow-hidden max-w-[260px] bg-black border ${
          isHighlighted ? 'border-yellow-400 ring-2 ring-yellow-400/40' : 'border-white/[0.06]'
        } ${msg.isRetrying ? 'opacity-50' : ''}`}>
    ...
    if (['pdf', 'file', 'office', 'text-file'].includes(type)) {
      return (
        <div className={`flex items-stretch max-w-[268px] bg-[#2a2a2a] rounded-[18px] border overflow-hidden ${
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-white/30 uppercase tracking-wider mt-0.5">{type.replace('-file', '').toUpperCase()}</p>
            </div>
          </button>
          <div className="w-px bg-white/[0.06]" />
          <button
            onClick={() => { if (!msg.isRetrying && !msg.isFailed) { const a = document.createElement('a'); a.href = msg.content; a.download = getFileName(msg.content); a.click(); } }}
    ...
    const renderHighlightedText = (text: string) => {
      ...
      return parts.map((part, i) =>
        part.toLowerCase() === searchQuery.toLowerCase() ? (
          <mark key={i} className={`${isCurrentSearch ? 'bg-yellow-300 text-black' : 'bg-yellow-200/40 text-white'} rounded px-0.5`}>
            {part}
          </mark>
        ) : part
      );
    ...
    <div className="flex flex-col h-[100dvh] text-white overflow-hidden relative" style={bgStyle}>
        ...
        <div className="flex items-center gap-0.5">
          <button
            ...
            className={`w-9 h-9 flex items-center justify-center rounded-[12px] transition-colors ${
              isSearching ? 'bg-[#FF203A]/15 text-[#FF203A]' : 'text-white/55 hover:bg-white/[0.07] hover:text-white'
            }`}
          >
          ...
          >
            ...
            {searchResults.length > 0 && (
              <div className="flex items-center gap-1.5 bg-[#2a2a2a] px-2.5 h-[38px] rounded-[13px] border border-white/[0.05]">
                ...
                <div className="flex items-center gap-0.5">
                  <button onClick={() => handleSearchMove('up')} className="p-1 hover:bg-white/[0.07] rounded-lg transition-colors">
                    <ChevronUp className="w-3.5 h-3.5 text-white/65" />
                  </button>
                  <button onClick={() => handleSearchMove('down')} className="p-1 hover:bg-white/[0.07] rounded-lg transition-colors">
                    <ChevronDown className="w-3.5 h-3.5 text-white/65" />
                  </button>
                </div>
              </div>
            )}
            <button onClick={handleCloseSearch} className="text-[13px] text-white/38 hover:text-white/70 px-1 whitespace-nowrap transition-colors">
              닫기
            </button>
          </motion.div>
        )}
        ...
        {timeCapsuleNotice && (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-semibold text-orange-300">{timeCapsuleNotice.receiver_name}님 타임캡슐</p>
                <p className="text-[11px] text-orange-400/60 mt-0.5">잠금 해제까지 {timeRemaining || getTimeUntilUnlock()}</p>
              </div>
              <div className="text-[11px] text-orange-400/70 font-mono bg-orange-500/10 px-2.5 py-1 rounded-full">
                {new Date(timeCapsuleNotice.unlock_at).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
              </div>
            </div>
          </motion.div>
        )}
              ...
              return (
                ...
                >
                  ...
                  <div className={`max-w-[73%] flex flex-col gap-[3px] ${isMe ? 'items-end' : 'items-start'}`}>
                    {/* 그룹 발신자 이름 */}
                    {!isMe && isGroupChat && (
                      <span className="text-[11.5px] text-white/35 ml-1">{sender?.name || '알 수 없는 사용자'}</span>
                    )}
                    ...
                    <div className="relative">
                      ...
                      {msg.isFailed && isMe && (
                        <button
                          onClick={() => { setFailedMessage(msg); setShowRetryModal(true); }}
      ...
      <BottomSheet isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} maxH="max-h-[38vh]">
        <div className="px-5 pt-1 pb-8">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              { ref: fileInputRef,   Icon: ImageIcon, label: '앨범',    color: 'from-blue-500/20 to-blue-600/10',   iconColor: 'text-blue-400' },
              { ref: cameraInputRef, Icon: Camera,    label: '카메라',  color: 'from-green-500/20 to-green-600/10', iconColor: 'text-green-400' },
              { ref: docInputRef,    Icon: FileText,  label: '파일',    color: 'from-purple-500/20 to-purple-600/10', iconColor: 'text-purple-400' },
              ...
              >
                <div className={`w-14 h-14 rounded-[18px] bg-gradient-to-br ${color} border border-white/[0.07] flex items-center justify-center`}>
      ...
      <BottomSheet isOpen={showRetryModal && !!failedMessage} onClose={() => { setShowRetryModal(false); setFailedMessage(null); }} maxH="max-h-[44vh]">
        <div className="px-6 pt-4 pb-3 text-center">
          ...
          <p className="text-[13.5px] text-white/38 leading-relaxed">메시지 전송에 실패했습니다.<br />다시 시도하시겠습니까?</p>
        </div>
    ...
    <div className="fixed inset-0 z-[100] flex flex-col justify-center overflow-hidden bg-black/98 backdrop-blur-2xl">
      ...
      <div className="flex-1 flex items-center justify-center relative w-full h-full">
        <AnimatePresence initial={false} custom={direction} mode="popLayout">
          <motion.img
            key={index} src={images[index]} custom={direction}
            variants={{
